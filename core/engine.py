...
class ULMEngine:
    ...
    def __init__(
        self, 
        target_yaml: Optional[Union[str, Path]] = None, 
    ...
    def merge_and_reconfigure(
        self, 
        current_state: Dict[str, Any], 
        new_data: List[Dict[str, Any]]
    ) -> Tuple[Dict[str, Any], int]:
        """Merges new log logs into the main YAML state database."""
        updated_chats: Dict[str, Any] = current_state.get("chats", {})
        mutations: int = 0
        
        for session in new_data:
            c_id: str = session["chat_id"]
            incoming_logs: List[Dict[str, Any]] = session["messages"]
            last_mutated: str = session["last_mutated"]
            ...
            if c_id not in updated_chats:
                ...
                existing_logs: List[Dict[str, Any]] = updated_chats[c_id].get("log", [])
    ...
    async def merge_and_reconfigure_async(
        self, 
        current_state: Dict[str, Any], 
        new_data: List[Dict[str, Any]]
    ) -> Tuple[Dict[str, Any], int]:
        """Asynchronous wrapper for non-blocking ETL pipeline execution."""
        return await asyncio.to_thread(self.merge_and_reconfigure, current_state, new_data)
